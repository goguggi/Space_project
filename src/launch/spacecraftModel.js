// 심우주 탐사선 3D 형상 (20단계, D-78)
// 역할: 항행 중 3인칭·광역에서 보이는 우주선을 만든다. 계산은 하지 않는다.
//
// 구성 (실제 심우주 탐사선의 공통 요소):
//   기밀 모듈(승무원이 타는 원통) · 앞쪽 도킹 포트 · 태양전지판 2장 · 고이득 접시 안테나 ·
//   방열판 2장 · 추진부(노즐 4기) · 자세 제어 추력기 · 항법등
// 외부 모델 파일을 쓰지 않고 기본 도형만 조합한다 (저장소 원칙).
// 좌표: +Z가 진행 방향, +Y가 위쪽. 전체 길이 약 24 단위.
...
export function createSpacecraft() {
  ...
  for (const side of [-1, 1]) {
    const arm = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 3.2, 8), dark);
    arm.rotation.z = Math.PI / 2;
    arm.position.set(side * 3.6, 0, -2);
    ship.add(arm);
    for (let i = 0; i < 2; i += 1) {
      const panel = new THREE.Mesh(new THREE.PlaneGeometry(7.4, 4.6), panelMat);
      panel.position.set(side * (8.9 + i * 7.5), 0, -2);
      panel.rotation.y = Math.PI / 2 * 0;   // 진행 방향과 나란히 눕힌다
      panel.rotation.x = Math.PI / 2;
      ship.add(panel);
      const spar = new THREE.Mesh(new THREE.BoxGeometry(7.5, 0.12, 0.12), dark);
      spar.position.set(side * (8.9 + i * 7.5), 0, -2);
      ship.add(spar);
    }
  }
