...
| `earthTexture.js` | 캔버스로 그린 지구 표면 텍스처와 구름 텍스처, 발사장 좌표에 맞춘 회전각 (D-69, D-87) |
...
- `landingScene.js`의 `setLiftoff({altitude, speed})`: 착륙의 반대. 다리를 접고 엔진을 최대로 켠 채 솟아오른다 (D-80).
- `cruiseScene.js`: 귀환 구간에서 우주선 기수를 180° 돌린다 (D-81). 광역 시점 배율 46 → 16 (D-83).

### 20단계 추가 (우주인 캐릭터 조작, D-84)

`landingScene.js`의 `stepWalk(input, dt, params)`는 일반 3D 게임의 캐릭터 컨트롤러다.

| 요소 | 방식 |
|---|---|
| 시선 | 화면을 누르면 포인터 잠금. 마우스 좌우가 `camYaw`, 상하가 `pitch`(±78°) |
| 이동 | WASD는 **카메라 기준**. 앞/뒤와 좌우 스트레이프 |
| 가속 | 목표 속도에 지수적으로 붙는다 (지면 14/s, 감속 11/s, 공중 제어 0.28) |
| 몸의 방향 | 1인칭은 시선을 그대로 따라간다. 3인칭은 실제 이동 방향으로 초당 9의 비율로 서서히 돈다 |
| 점프 | 지면에서만. 중력은 그 천체의 값. 착지하면 카메라가 살짝 내려앉는다 |
| 카메라 | 1인칭은 눈높이 1.72 m에 머리 흔들림. 3인칭은 시선 뒤 7.5 m를 초당 12의 비율로 부드럽게 따라간다 |

### 21단계 추가 (연출 다듬기)

| 파일 | 내용 |
|---|---|
| `padPlume.js` (새 파일) | 이륙 순간 발사대에서 옆으로 쏟아지는 연기·수증기 구름. 알갱이 54개를 뿜어 퍼뜨린다 (D-91) |
| `earthPhoto.js` (새 파일) | NASA 블루마블 사진을 찾아 텍스처로 돌려준다. 못 찾으면 `null` → 캔버스 그림을 쓴다 (D-88) |
| `rocketModel.js` | 화염을 네 겹(연기·주황·노랑·백열심) + 마하 디스크 4개 + 노즐 발광 스프라이트 + 실제 PointLight로. 고도가 오를수록 배기가 최대 2.4배까지 벌어진다 |
| `earthTexture.js` | 수심 얼룩·대륙붕·기후대 색 띠·산맥 얼룩·들쭉날쭉한 극지방 얼음을 캔버스 합성으로 층층이 쌓는다. `createCloudTexture()`가 구름층 텍스처를 따로 만든다 (D-87) |
| `launchScene.js` | 지표보다 7.6 km 높은 구름 구를 추가하고 아주 느리게 돌린다. NASA 사진을 찾으면 지구 텍스처를 조용히 바꿔 끼운다 |

지구 그림은 픽셀을 하나씩 계산하지 않는다. 작은 잡음 그림을 만들어 크게 늘려 겹치고(캔버스 확대 보간),
