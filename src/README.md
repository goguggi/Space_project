...
- `main.js` (1단계. 2단계 목적지, 3단계 속도·프리셋·로런츠, 4단계 편도/왕복과 `recompute()`, 5단계 결과 표, 6단계 스톱워치, 7단계 출발 나이·막대 그래프·생존 표와 `renderLifespan()` 연결, 10~11단계 발사 장면 조립(`launchController`) 동적 import, 15단계 임무 시계(`setProgress`)로 3D·스톱워치·막대 그래프·생존 표를 함께 구동)
