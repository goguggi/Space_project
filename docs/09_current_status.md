...
| 검증 | `tests/physics.test.html` 120 / 120 통과 |
...
| 21c | 발사·항행에 같은 우주선(회전 거주 구역 포함), 경유 여행(지구 → 경유지 → 목적지 → 지구) | `src/launch/spacecraftModel.js`, `rocketModel.js`, `src/physics/route.js`, `src/ui/waypointSelector.js` |
| 21b | 실제 궤도 투입 곡선(중력 선회 + 원궤도 속도 SECO)과 HUD 상승 속도, 항행 화면 지구에 NASA 실사 텍스처, 도착 요약 카드 | `src/physics/launchDynamics.js`, `src/launch/launchHud.js`, `cruiseScene.js`, `src/ui/arrivalCard.js` |
...
20단계에서 알게 된 것:
- 진행 막대에 항행 진행률만 쓰면 착륙·탐사 중에 100%로 보여 "이미 도착"으로 오해된다. 단계마다 알맞은 진행률과 이름을 보여야 한다 (D-85).
